...
int main(int argc, char *argv[]){
    ...
    string write_file = "./dataset/meanShape.yaml";
    FileStorage fsw( write_file, FileStorage::APPEND );
    // fsw << "delaunay" << "[";
    // for( int i = 0; i < triangle_list.size(); ++i ){
    //     fsw << "{:" << "points" << triangle_list[i] ;
    //     fsw << "}";
    // }
    // fsw << "]";
    ...
    int t = 0;
    fsw << "delaunay_index" << "[";
    for( auto index:index_triangle ){
        fsw << "{:" << "t" << ++t << "index" << (Vec3i)index;
        fsw << "}";
    }
    fsw << "]";
